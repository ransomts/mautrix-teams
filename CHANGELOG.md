...
* Channel system messages (`ThreadActivity/*`: member added, topic changed) are dropped instead of being bridged as garbled text from a ghost minted from the channel's own thread ID; `@thread.tacv2` senders are recognised as threads.
...
* The user's own membership and profile changes in a portal no longer log "membership change target is not a ghost".
